...
def test_has_been_edited_returns_true_when_any_segment_edited(sample_run):
    from transcript_editor import has_been_edited, save_edits

    assert has_been_edited(str(sample_run["json_path"])) is False
    save_edits(
        str(sample_run["json_path"]),
        ["CHANGED.", "Let's dive into this autopoiesis thing.", "Sounds good to me."],
    )
...
def test_save_edits_creates_backup_on_first_call(sample_run):
    ...
    backup_path = json_path.with_name(
        json_path.stem.replace(".whisperx", "") + ".whisperx.original.json"
    )
...
def test_save_edits_does_not_overwrite_backup_on_second_call(sample_run):
    ...
    save_edits(
        str(json_path), ["First.", "Let's dive into this autopoiesis thing.", "Sounds good to me."]
    )
    first_backup_content = backup_path.read_text(encoding="utf-8")
    result = save_edits(
        str(json_path), ["Second.", "Let's dive into this autopoiesis thing.", "Sounds good to me."]
    )
